...
export function PatientPortalAppointmentDetailsPage() {
  ...
  return <div className="space-y-6"><PageHeader title="Detalle de cita" description={`${item.scheduled_date} ${item.start_time}`} /><Card><div className="grid gap-4 md:grid-cols-2"><Info label="Médico" value={appointmentDoctor(item)} /><Info label="Modalidad" value={item.modality === "online" ? "En línea" : "Presencial"} /><Info label="Motivo" value={item.reason} /><Info label="Estado" value={item.status} /><Info label="Notas" value={item.notes} /></div></Card></div>;
}
...
export function PatientRequestAppointmentPage() {
  ...
  async function submit(e: FormEvent) { e.preventDefault(); try { await requestPatientAppointment(form); toast.success("Solicitud de cita enviada correctamente."); setForm({ specialty: "", doctor: "", scheduled_date: "", start_time: "", reason: "", modality: "presencial" }); setAvailability(null); } catch (error) { toast.error(getErrorMessage(error)); } }
  const filteredDoctors = form.specialty ? doctors.filter((d) => String(d.specialty) === form.specialty) : doctors;
  return <form className="space-y-6" onSubmit={submit}><PageHeader title="Solicitar cita" description="Selecciona médico, fecha, modalidad y horario disponible." actions={<Button>Solicitar</Button>} /><Card><div className="grid gap-3 md:grid-cols-2"><select className="h-11 rounded-md border px-3 text-sm" value={form.specialty} onChange={(e) => setForm({ ...form, specialty: e.target.value, doctor: "", start_time: "" })}><option value="">Especialidad</option>{specialties.map((s) => <option key={s.id} value={s.id}>{s.nombre}</option>)}</select><select className="h-11 rounded-md border px-3 text-sm" required value={form.doctor} onChange={(e) => setForm({ ...form, doctor: e.target.value, start_time: "" })}><option value="">Médico</option>{filteredDoctors.map((d) => <option key={d.id} value={d.id}>{String(d.user_nombre ?? d.id)}</option>)}</select><input className="h-11 rounded-md border px-3 text-sm" required type="date" value={form.scheduled_date} onChange={(e) => setForm({ ...form, scheduled_date: e.target.value, start_time: "" })} /><div className="grid gap-2 rounded-md border border-slate-200 p-3 text-sm md:col-span-2"><p className="font-semibold text-slate-700">Modalidad</p><div className="grid gap-2 sm:grid-cols-2"><button type="button" className={`rounded-md border px-4 py-3 text-left ${form.modality === "presencial" ? "border-brand-600 bg-brand-50 text-brand-800" : "border-slate-200"}`} onClick={() => { setForm({ ...form, modality: "presencial", start_time: "" }); setAvailability(null); }}><strong>Presencial</strong><span className="block text-xs text-slate-500">Atención en la clínica</span></button><button type="button" className={`rounded-md border px-4 py-3 text-left ${form.modality === "online" ? "border-brand-600 bg-brand-50 text-brand-800" : "border-slate-200"}`} onClick={() => { setForm({ ...form, modality: "online", start_time: "" }); setAvailability(null); }}><strong>En línea</strong><span className="block text-xs text-slate-500">Atención virtual si la clínica lo permite</span></button></div></div><Button type="button" variant="outline" onClick={check}>Consultar disponibilidad</Button><select className="h-11 rounded-md border px-3 text-sm" required value={form.start_time} onChange={(e) => setForm({ ...form, start_time: e.target.value })}><option value="">Horario</option>{availability?.available_slots.map((slot) => <option key={slot.start_time} value={slot.start_time}>{slot.start_time} - {slot.end_time}</option>)}</select><textarea className="min-h-28 rounded-md border px-3 py-2 text-sm md:col-span-2" required placeholder="Motivo de consulta" value={form.reason} onChange={(e) => setForm({ ...form, reason: e.target.value })} /></div></Card></form>;
}
