...
export function InventoryItemsPage({ doctorOnly = false }: { doctorOnly?: boolean }) {
  ...
  const [movementKey, setMovementKey] = useState(operationKey);
  ...
  async function submitMovement(e: FormEvent) {
    ...
    try {
      if (movementType === "entrada") {
        await stockIn(movementItem.id, { quantity: movementForm.quantity, reason: movementForm.reason, unit_cost: movementForm.unit_cost, lot_number: movementForm.lot_number, expiration_date: movementForm.expiration_date, notes: movementForm.notes, idempotency_key: movementKey }, movementKey);
      } else {
        await stockOut(movementItem.id, { quantity: movementForm.quantity, reason: movementForm.reason, notes: movementForm.notes, idempotency_key: movementKey }, movementKey);
      }
...
export function InventoryLotsPage() {
  ...
  return <div className="space-y-6"><PageHeader title="Lotes" description="Control de cantidades y vencimientos." /><Card>{items.length ? <Table data={items} columns={[{ key: "item", header: "Producto", render: (i) => i.item_nombre }, { key: "lot", header: "Lote", render: (i) => i.lot_number }, { key: "exp", header: "Vence", render: (i) => i.expiration_date || "-" }, { key: "qty", header: "Cantidad", render: (i) => i.quantity_current }, { key: "state", header: "Estado", render: (i) => i.expired ? "Vencido" : i.expiring_soon ? "Por vencer" : "Vigente" }]} /> : <EmptyState title="No hay lotes disponibles para este producto." />}</Card></div>;
}
...
export function InventoryMovementsPage() {
  ...
  return <div className="space-y-6"><PageHeader title="Movimientos" description="Historial de entradas, salidas y ajustes." /><Card>{items.length ? <Table data={items} columns={[{ key: "date", header: "Fecha", render: (i) => i.creado_en?.slice(0, 10) }, { key: "item", header: "Producto", render: (i) => i.item_nombre }, { key: "lot", header: "Lote", render: (i) => i.lot_number || "-" }, { key: "type", header: "Tipo", render: (i) => <MovementTypeBadge type={i.movement_type} /> }, { key: "qty", header: "Cantidad", render: (i) => i.quantity }, { key: "balance", header: "Saldo", render: (i) => `${i.balance_before} -> ${i.balance_after}` }, { key: "reason", header: "Motivo", render: (i) => i.reason }, { key: "user", header: "Realizado por", render: (i) => i.performed_by_nombre || "-" }]} /> : <EmptyState title="No hay movimientos para los filtros seleccionados." />}</Card></div>;
}
