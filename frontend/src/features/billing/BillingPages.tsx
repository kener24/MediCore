...
export function CashPage() {
  ...
  return <div className="space-y-6"><PageHeader title="Caja" description="Apertura, cierre y movimientos." actions={current ? <Button variant="danger" onClick={showClose}>Cerrar caja</Button> : <Button onClick={showOpen}>Abrir caja</Button>} />{current ? <Card title="Caja actual" actions={<div className="flex gap-2"><Button variant="outline" onClick={() => showMovement("ingreso")}>Ingreso</Button><Button variant="outline" onClick={() => showMovement("egreso")}>Egreso</Button></div>}><div className="grid gap-4 md:grid-cols-3"><StatCard label="Apertura" value={money(current.opening_amount)} icon={<DollarSign className="h-5 w-5" />} /><StatCard label="Esperado" value={money(current.expected_amount)} icon={<DollarSign className="h-5 w-5" />} /><StatCard label="Movimientos" value={current.movements?.length ?? 0} icon={<DollarSign className="h-5 w-5" />} /></div></Card> : null}<Card title="Sesiones"><Table data={sessions} columns={[{ key: "id", header: "ID", render: (i) => i.id }, { key: "user", header: "Usuario", render: (i) => i.opened_by_nombre }, { key: "open", header: "Apertura", render: (i) => money(i.opening_amount) }, { key: "expected", header: "Esperado", render: (i) => money(i.expected_amount) }, { key: "diff", header: "Diferencia", render: (i) => money(i.difference_amount) }, { key: "status", header: "Estado", render: (i) => <CashStatusBadge status={i.status} /> }]} /></Card><Modal open={Boolean(cashModal)} title={cashModal === "open" ? "Abrir caja" : cashModal === "close" ? "Cerrar caja" : "Registrar movimiento"} onClose={() => setCashModal(null)} actions={<><ModalCloseButton onClick={() => setCashModal(null)} /><Button form="cash-form" type="submit">{cashModal === "open" ? "Abrir" : cashModal === "close" ? "Cerrar" : "Registrar"}</Button></>}><form id="cash-form" className="grid gap-4" onSubmit={submitCash}>{cashModal === "open" ? <label className="block space-y-1.5"><span className="text-sm font-medium text-slate-700">Monto de apertura</span><input className="h-11 w-full rounded-md border border-slate-300 px-3 text-sm" min="0" required step="0.01" value={cashForm.opening_amount} onChange={(e) => setCashForm({ ...cashForm, opening_amount: cleanDecimal(e.target.value) })} /></label> : null}{cashModal === "close" ? <><p className="text-sm text-slate-600">Monto esperado: <b>{money(current?.expected_amount)}</b></p><label className="block space-y-1.5"><span className="text-sm font-medium text-slate-700">Monto de cierre</span><input className="h-11 w-full rounded-md border border-slate-300 px-3 text-sm" min="0" required step="0.01" value={cashForm.closing_amount} onChange={(e) => setCashForm({ ...cashForm, closing_amount: cleanDecimal(e.target.value) })} /></label><label className="block space-y-1.5"><span className="text-sm font-medium text-slate-700">Notas</span><textarea className="min-h-24 w-full rounded-md border border-slate-300 px-3 py-2 text-sm" value={cashForm.notes} onChange={(e) => setCashForm({ ...cashForm, notes: e.target.value })} /></label></> : null}{cashModal === "movement" ? <><label className="block space-y-1.5"><span className="text-sm font-medium text-slate-700">Tipo</span><select className="h-11 w-full rounded-md border border-slate-300 px-3 text-sm" value={cashForm.movement_type} onChange={(e) => setCashForm({ ...cashForm, movement_type: e.target.value })}><option value="ingreso">Ingreso</option><option value="egreso">Egreso</option></select></label><label className="block space-y-1.5"><span className="text-sm font-medium text-slate-700">Monto</span><input className="h-11 w-full rounded-md border border-slate-300 px-3 text-sm" min="0.01" required step="0.01" value={cashForm.amount} onChange={(e) => setCashForm({ ...cashForm, amount: cleanDecimal(e.target.value) })} /></label><label className="block space-y-1.5"><span className="text-sm font-medium text-slate-700">Razon</span><input className="h-11 w-full rounded-md border border-slate-300 px-3 text-sm" required value={cashForm.reason} onChange={(e) => setCashForm({ ...cashForm, reason: e.target.value })} /></label><label className="block space-y-1.5"><span className="text-sm font-medium text-slate-700">Notas</span><textarea className="min-h-24 w-full rounded-md border border-slate-300 px-3 py-2 text-sm" value={cashForm.notes} onChange={(e) => setCashForm({ ...cashForm, notes: e.target.value })} /></label></> : null}</form></Modal></div>;
}