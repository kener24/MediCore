...
def clinic_local_date(clinic):
    settings = get_or_create_clinic_settings(clinic)
    try:
        return timezone.now().astimezone(ZoneInfo(settings.timezone)).date()
    except (ValueError, ZoneInfoNotFoundError):
        return timezone.localdate()


def admin_metric_period(request, clinic):
    ...
    return {
        ...
        "date_from": date_from,
        "date_to": date_to,
        "timezone": get_or_create_clinic_settings(clinic).timezone,
    }
...
def build_clinic_admin_dashboard(request, clinic):
    period = admin_metric_period(request, clinic)
    ...
    fiscal_profile = ClinicFiscalProfile.objects.filter(clinic=clinic).first()
    active_range = FiscalDocumentRange.objects.filter(
        clinic=clinic,
    ...
    open_cash = CashSession.objects.filter(clinic=clinic, status=CashSession.Status.ABIERTA, active=True)
    cash_differences = CashSession.objects.filter(
        clinic=clinic,
    ...
    return {
        ...
        "operation": {
            ...
            "appointments_attended": appointments.filter(status=Appointment.Status.ATENDIDA).count(),
            "appointments_cancelled": appointments.filter(status=Appointment.Status.CANCELADA).count(),
            "appointments_no_show": appointments.filter(status=Appointment.Status.NO_ASISTIO).count(),
            "patients_waiting": visits.filter(status__in=[PatientVisit.Status.WAITING_TRIAGE, PatientVisit.Status.WAITING_DOCTOR]).count(),
            "patients_in_consultation": visits.filter(status=PatientVisit.Status.IN_CONSULTATION).count(),
            "patients_hospitalized": Hospitalization.objects.filter(clinic=clinic, status__in=Hospitalization.ACTIVE_STATUSES).count(),
        },
        "finance": {
            "invoiced": _money_total(invoices, "total_amount"),
            "paid": _money_total(payments, "amount"),
            "balance_due": _money_total(invoices, "balance_due"),
            "open_cash_sessions": open_cash.count(),
            "cash_differences": cash_differences.count(),
        },
        "inventory": {
            "out_of_stock": inventory.filter(stock_current=0).count(),
            "low_stock": inventory.filter(stock_current__gt=0, stock_current__lte=F("stock_minimum")).count(),
            "expiring_lots": lots.filter(expiration_date__range=(today, expiring_limit)).count(),
        ...
        "operation_status": {
            ...
            "cash_open": open_cash.exists(),
            "valid_fiscal_range": bool(active_range),
            "fiscal_billing_enabled": bool(fiscal_profile and fiscal_profile.is_fiscal_billing_enabled),
