...
import type { PurchaseOrder, PurchaseReceipt, PurchaseReturn, PurchaseStats, Supplier } from "../types/purchase";

type ListResponse<T> = T[] | { results: T[] };
const list = <T>(data: ListResponse<T>) => Array.isArray(data) ? data : data.results ?? [];

export async function getSuppliers(filters?: Record<string, string>) { const { data } = await api.get<ListResponse<Supplier>>("/purchases/suppliers/", { params: filters }); return list(data); }
...
export async function getPurchaseOrders(filters?: Record<string, string>) { const { data } = await api.get<ListResponse<PurchaseOrder>>("/purchases/orders/", { params: filters }); return list(data); }
...
export async function getPurchaseReceipts(filters?: Record<string, string>) { const { data } = await api.get<ListResponse<PurchaseReceipt>>("/purchases/receipts/", { params: filters }); return list(data); }
...
export async function receivePurchaseOrder(orderId: number | string, payload: Record<string, unknown>, idempotencyKey?: string) { const { data } = await api.post<PurchaseReceipt>(`/purchases/orders/${orderId}/receive/`, payload, { headers: idempotencyKey ? { "Idempotency-Key": idempotencyKey } : undefined }); return data; }
export async function reversePurchaseReceipt(receiptId: number | string, reason: string) { const { data } = await api.post<PurchaseReceipt>(`/purchases/receipts/${receiptId}/reverse/`, { reason }); return data; }
export async function returnPurchaseItems(receiptId: number | string, payload: { reason: string; idempotency_key: string; items: Array<{ receipt_item: number; quantity: string }> }) { const { data } = await api.post<PurchaseReturn>(`/purchases/receipts/${receiptId}/return-items/`, payload, { headers: { "Idempotency-Key": payload.idempotency_key } }); return data; }
export async function getPurchaseReturns(filters?: Record<string, string>) { const { data } = await api.get<ListResponse<PurchaseReturn>>("/purchases/returns/", { params: filters }); return list(data); }
