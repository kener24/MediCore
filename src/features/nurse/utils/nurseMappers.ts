...
function fullName(source?: Record<string, unknown> | null) {
  ...
  const direct = pick<string>(source, ['nombre_completo', 'full_name', 'name', 'nombre']);
...
export function mapNursePatient(raw: unknown): NursePatientSummary {
  ...
  return {
    ...
    document: pick<string>(patient, ['documento', 'identidad', 'dni', 'document', 'rtn']),
    ...
    reason: pick<string>(source, ['reason', 'motivo', 'chief_complaint', 'queja_principal', 'motivo_consulta']),
    ...
    doctorName: pick<string>(source, ['doctor_name', 'medico_nombre']),
  };
}
...
export function mapVitalSigns(raw: unknown): NurseVitalSigns {
  ...
  return {
    ...
    systolicPressure: asNumber(pick(source, ['systolic_pressure', 'presion_sistolica', 'systolic'])),
    diastolicPressure: asNumber(pick(source, ['diastolic_pressure', 'presion_diastolica', 'diastolic'])),
    ...
    heightCm: asNumber(pick(source, ['height_cm', 'talla', 'talla_cm', 'height'])),
...
export function mapTriage(raw: unknown): NurseTriage {
  ...
  return {
    ...
    chiefComplaint: pick<string>(source, ['chief_complaint', 'queja_principal', 'motivo_consulta']),
    initialAssessment: pick<string>(source, ['initial_assessment', 'evaluacion_inicial', 'assessment']),
    notes: pick<string>(source, ['notes', 'notas', 'observaciones']),
    vitalSigns: vital ? mapVitalSigns(vital) : null,
    nurseName: pick<string>(source, ['nurse_name', 'enfermera_nombre']),
    doctorName: pick<string>(source, ['doctor_name', 'medico_nombre']),
    completedAt: pick<string>(source, ['completed_at', 'fecha_completado']),
