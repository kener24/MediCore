...
export function SuperAdminClinicsScreen() {
  ...
  const confirmStatus = (clinic: SuperAdminClinic, active: boolean) => {
    Alert.alert(active ? 'Activar clínica' : 'Desactivar clínica', `${clinicName(clinic)} cambiará de estado. ¿Deseas continuar?`, [
      { text: 'Cancelar', style: 'cancel' },
      { text: 'Confirmar', onPress: () => void changeStatus(clinic, active), style: active ? 'default' : 'destructive' },
    ]);
  };

  async function changeStatus(clinic: SuperAdminClinic, active: boolean) {
    try {
      await setClinicActive(clinic.id, active);
      await load(true);
    } catch (err) {
        ...
        <ScrollView contentContainerStyle={styles.content} refreshControl={<RefreshControl onRefresh={() => void load(true)} refreshing={refreshing} />}>
          ...
          {filtered.map((clinic) => (
            <AppCard key={clinic.id} style={styles.card}>
              ...
              <AppButton label={clinic.activo === false ? 'Activar' : 'Desactivar'} onPress={() => confirmStatus(clinic, clinic.activo === false)} variant={clinic.activo === false ? 'secondary' : 'danger'} />
            </AppCard>
          ))}
        </ScrollView>
      </SafeAreaView>
