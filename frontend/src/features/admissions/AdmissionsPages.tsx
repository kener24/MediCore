...
export function TriageQueuePage() {
  ...
  const [selectedHistory, setSelectedHistory] = useState<PatientVisit | null>(null);
  ...
  return <div className="space-y-6"><PageHeader title="Triaje" description="Evaluación inicial, signos vitales y prioridad clínica." actions={<Button variant="outline" onClick={() => void load()}>Actualizar</Button>} />{error ? <Card><div className="space-y-3 text-center"><p className="font-semibold text-red-700">No se pudo cargar la información.</p><p className="text-sm text-slate-600">{error}</p><Button variant="outline" onClick={() => void load()}>Reintentar</Button></div></Card> : <><Card title="Cola de triaje"><VisitTable visits={visits} actions={(v) => <div className="flex flex-wrap gap-2">{v.status === "waiting_triage" ? <Button className="h-8 px-3 text-xs" isLoading={busyId === v.id} variant="outline" onClick={() => void begin(v)}>Iniciar triaje</Button> : null}{v.status === "in_triage" ? <Button className="h-8 px-3 text-xs" variant="outline" onClick={() => setSelectedSigns(v)}>{v.vital_signs ? "Corregir signos" : "Registrar signos"}</Button> : null}{v.status === "in_triage" ? <Button className="h-8 px-3 text-xs" disabled={!v.vital_signs} onClick={() => v.vital_signs ? setSelectedComplete(v) : toast.error("Registra signos vitales antes de completar el triaje.")}>Completar</Button> : null}</div>} /></Card><Card title="Triajes completados"><VisitTable visits={completed} actions={(v) => <Button className="h-8 px-3 text-xs" variant="outline" onClick={() => setSelectedHistory(v)}>Ver detalle</Button>} /></Card></>}<Modal open={Boolean(selectedSigns)} title={`Signos vitales · ${selectedSigns?.patient_nombre ?? ""}`} onClose={() => setSelectedSigns(null)} actions={<ModalCloseButton onClick={() => setSelectedSigns(null)} />}>{selectedSigns ? <VitalSignsMiniForm visit={selectedSigns} onSaved={async () => { setSelectedSigns(null); await load(); }} /> : null}</Modal><Modal open={Boolean(selectedComplete)} title={`Completar triaje · ${selectedComplete?.patient_nombre ?? ""}`} onClose={() => setSelectedComplete(null)} actions={<ModalCloseButton onClick={() => setSelectedComplete(null)} />}>{selectedComplete ? <CompleteTriageForm visit={selectedComplete} onCompleted={async () => { setSelectedComplete(null); await load(); }} /> : null}</Modal><Modal open={Boolean(selectedHistory)} title={`Triaje completado · ${selectedHistory?.patient_nombre ?? ""}`} onClose={() => setSelectedHistory(null)} actions={<ModalCloseButton onClick={() => setSelectedHistory(null)} />}>{selectedHistory ? <div className="grid gap-4 text-sm"><div className="grid gap-3 rounded-md border bg-slate-50 p-4 md:grid-cols-2"><p><b>Visita:</b> {selectedHistory.visit_number}</p><p><b>Prioridad:</b> {{ normal: "Normal", priority: "Prioritario", urgent: "Urgente", emergency: "Emergencia" }[selectedHistory.priority]}</p><p><b>Inicio:</b> {selectedHistory.triage_started_at ? new Date(selectedHistory.triage_started_at).toLocaleString("es-HN") : "No disponible"}</p><p><b>Finalización:</b> {selectedHistory.triage_completed_at ? new Date(selectedHistory.triage_completed_at).toLocaleString("es-HN") : "No disponible"}</p><p><b>Enfermería:</b> {selectedHistory.assigned_nurse_nombre || "No disponible"}</p><p><b>Médico:</b> {selectedHistory.assigned_doctor_nombre || "Sin asignar"}</p></div><div><p className="font-semibold">Motivo principal</p><p className="mt-1 text-slate-600">{selectedHistory.chief_complaint || selectedHistory.reason}</p></div><div><p className="font-semibold">Evaluación inicial</p><p className="mt-1 text-slate-600">{selectedHistory.initial_assessment || selectedHistory.symptoms || "No registrada"}</p></div><div className="grid gap-3 md:grid-cols-2"><div className="rounded-md border border-rose-200 bg-rose-50 p-3"><p className="font-semibold text-rose-800">Alergias</p><p className="text-rose-700">{selectedHistory.patient_alergias || "No hay alergias registradas."}</p></div><div className="rounded-md border p-3"><p className="font-semibold">Antecedentes críticos</p><p className="text-slate-600">{selectedHistory.patient_enfermedades_cronicas || "No hay antecedentes críticos registrados."}</p></div></div>{selectedHistory.vital_signs ? <div className="grid gap-2 rounded-md border p-4 md:grid-cols-3"><p><b>Temperatura:</b> {selectedHistory.vital_signs.temperature ?? "-"} °C</p><p><b>Presión:</b> {selectedHistory.vital_signs.blood_pressure_systolic ?? "-"}/{selectedHistory.vital_signs.blood_pressure_diastolic ?? "-"}</p><p><b>Frecuencia cardíaca:</b> {selectedHistory.vital_signs.heart_rate ?? "-"}</p><p><b>Frecuencia respiratoria:</b> {selectedHistory.vital_signs.respiratory_rate ?? "-"}</p><p><b>Saturación:</b> {selectedHistory.vital_signs.oxygen_saturation ?? "-"}%</p><p><b>IMC:</b> {selectedHistory.vital_signs.bmi ?? "-"}</p><p><b>Dolor:</b> {selectedHistory.vital_signs.pain_scale ?? "-"}/10</p><p className="md:col-span-2"><b>Observaciones:</b> {selectedHistory.vital_signs.notes || "Sin observaciones"}</p></div> : <EmptyState title="Sin signos vitales" description="No hay signos vitales asociados a este triaje." />}</div> : null}</Modal></div>;
}
